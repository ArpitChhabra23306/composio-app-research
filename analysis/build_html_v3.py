"""
build_html_v3.py — Builds the final index.html from results_v3.json (or falls back to v2).
...
def build():
    ...
    v3 = load_json("results_v3.json")
    v2 = load_json("results_v2.json", [])
    v1 = load_json("results_v1.json", [])
    patterns = load_json("patterns.json", {})
    verify_v3 = load_json("verification_v3.json")
    verify_manual = load_json("verification_manual_v3.json")  # human-verified GT
    verify_v2 = load_json("verification_sample_v2.json", [])
    accuracy_all = load_json("accuracy_all.json", {})

    results = v3 if v3 else v2
    version_label = "V3 (Post-Claude Fix)" if v3 else "V2"
    results_file  = "results_v3.json" if v3 else "results_v2.json"
    ...
    v3_stats = verify_manual.get("accuracy_stats", {}) if verify_manual else (
               verify_v3.get("accuracy_stats", {}) if verify_v3 else {})
    ...
    verify_source = verify_manual or verify_v3
    verify_rows = ""
    if verify_source:
...
<span class="ok">STEP 4: VALIDATE</span>  build_result_record()
  ...
  |
  v
<span class="ok">STEP 5: PERSIST</span>  Incremental write to results_v3.jsonl (crash-safe)
       → Final write to results_v3.json
       → patterns.py aggregates distributions per category
  |
  v
<span class="ok">STEP 6: VERIFY</span>   verify_v3.py — stratified 20-app sample
...
<footer>
  <p>Built with <strong>Composio SDK</strong> + <strong>gpt-4o</strong> | 
     ...
     Generated: {now}</p>
  <p style="margin-top:8px">Results file: {results_file} &nbsp;|&nbsp; Verification: {len(verify_v3.get('verifications', [])) if verify_v3 else 'pending'} apps checked</p>
</footer>
